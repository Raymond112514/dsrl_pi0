#!/usr/bin/env python3
"""Launch VAE-latent residual RL (a = a_base + λ * decode(w))."""

import argparse
import sys

from examples.train_sim_vae import main
from jaxrl2.utils.launch_util import parse_training_args


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Residual RL in VAE latent space (comparison to PCA eigenbasis)."
    )
    ...
    parser.add_argument(
        ...
        default=20,
        type=int,
        help="Base-policy rollouts used to fit the action VAE before SAC updates",
    )
    parser.add_argument(
        "--vae_latent_dim",
        default=8,
        type=int,
        help="VAE latent dim (= SAC action dim). Default 8 to match PCA --num_basis.",
    )
    parser.add_argument("--num_basis", default=8, type=int, help="Alias for --vae_latent_dim")
    parser.add_argument("--vae_hidden", default=128, type=int)
    parser.add_argument("--vae_epochs", default=120, type=int)
    parser.add_argument("--vae_batch_size", default=256, type=int)
    parser.add_argument("--vae_lr", default=1e-3, type=float)
    parser.add_argument("--vae_beta", default=1e-3, type=float)
    parser.add_argument(
        "--vae_linear",
        action="store_true",
        help="Ablation: linear encoder/decoder (closer to PCA)",
    )
    parser.add_argument(
        "--vae_path",
        default="",
        type=str,
        help="Optional pre-fit residual_vae.pt (skips online VAE fit)",
    )
    parser.add_argument(
        "--basis_path",
        default="",
    ...
    variant.vae_latent_dim = int(getattr(variant, "vae_latent_dim", variant.num_basis))
