...
def collect_traj(variant, agent, env, i, agent_dp=None):
    ...
    if variant.env == 'libero':
        ...
        action_dim = 14  # Check actual dimension
    else:
    ...
    action_list = []
    obs_list = []
    last_diffusion_actions = None
    last_residual_actions = None
...
def perform_control_eval(agent, env, i, variant, wandb_logger, agent_dp=None):
    """
    Residual RL version: Evaluation with residual actions combined with base diffusion policy.
    """
    query_frequency = variant.query_freq
