...
def obs_to_img(obs, variant):
    ...
    if variant.resize_image > 0:
        curr_image = np.array(
            PIL.Image.fromarray(curr_image).resize((variant.resize_image, variant.resize_image))
        )
    return curr_image
...
def fit_basis_from_warmup_chunks(variant, warmup_chunks):
    ...
    if len(warmup_chunks) < 2:
        raise ValueError(f"Need >=2 action chunks for PCA, got {len(warmup_chunks)}")
    chunks = np.stack(warmup_chunks, axis=0)
    ...
    print_green(
        f"Fitted {basis.num_basis} PCs on {chunks.shape[0]} chunks from "
        f"{variant.warmup_rollouts} warmup trajectories "
        f"(explained variance={basis.explained_variance_ratio.sum():.3f})"
    )
    return basis
...
def trajwise_alternating_training_loop(
    ...
    num_traj = 0
    warmup_chunks = []
    warmup_rollouts = variant.warmup_rollouts
    ...
    with tqdm(total=variant.max_steps, initial=0) as pbar:
        while i <= variant.max_steps:
            traj = collect_traj(variant, agent, env, i, agent_dp, basis)
            num_traj += 1
            traj_id = online_replay_buffer._traj_counter
            add_online_data_to_buffer(variant, traj, online_replay_buffer)
            if basis is None:
                warmup_chunks.extend(traj["pi0_chunks"])
            ...
            if basis is None and num_traj >= warmup_rollouts:
            ...
            print_green(f"Updating at step {i}")
            for _ in range(num_gradsteps):
                if i == 0:
                    print("performing evaluation for initial checkpoint")
                    if perform_control_evals:
                        perform_control_eval(agent, eval_env, i, variant, wandb_logger, agent_dp, basis)
                ...
                if i % variant.eval_interval == 0:
                    ...
                    if hasattr(agent, "perform_eval"):
                        agent.perform_eval(
                            variant, i, wandb_logger, replay_buffer, replay_buffer_iterator, eval_env, None
                        )
...
def collect_traj(variant, agent, env, i, agent_dp=None, basis=None):
    ...
    num_basis = basis.num_basis if basis is not None else variant.num_basis
    ...
    for t in tqdm(range(max_timesteps)):
        ...
        if t % query_frequency == 0:
            ...
            chunk = np.asarray(diffusion_actions[:query_frequency], dtype=np.float64).reshape(-1)
            pi0_chunks.append(chunk)

        action_t = diffusion_actions[t % query_frequency]
        obs_dict["action_diffusion"] = action_t[None, :, None]

        if i == 0:
            actions_residual_coeffs = np.zeros(num_basis, dtype=np.float32)
        else:
            actions_residual_coeffs = agent.sample_actions(obs_dict)[0]

        action_list.append(actions_residual_coeffs)
        obs_list.append(obs_dict)
        if basis is None:
            residual_action_t = action_t
        else:
            residual_7d = basis.coeffs_to_step_residual(actions_residual_coeffs, t, residual_scale)
            residual_action_t = action_t + residual_7d

        if "libero" in variant.env:
            obs, reward, done, _ = env.step(residual_action_t)
        elif "aloha" in variant.env:
            obs, reward, terminated, truncated, _ = env.step(residual_action_t)
...
def perform_control_eval(agent, env, i, variant, wandb_logger, agent_dp=None, basis=None):
    ...
    for rollout_id in range(variant.eval_episodes):
        ...
        print(f"Rollout {rollout_id} : episode_return={np.sum(rewards)}, Success: {is_success}")
